...
@php
    ...
    $panels = [
        [
            'image' => 'phonics_l2/chrev/catch.png',
            'imageSize' => 'h-[20vw]',
            'options' => ['batch', 'catch'],
            'answer' => 'catch',
            'highlight' => 'ch',
            'audio' => 'ch_rev/catch.m4a',
            'tip' =>
                'Tip: Invite children to sound out each word by parts (e.g. /sh/.../ip/) <br> before selecting the word that matches the picture.',
        ],
        [
            'image' => 'phonics_l2/chrev/snatch.png',
            'imageSize' => 'w-[20vw]',
            'options' => ['snatch', 'fetch'],
            'answer' => 'snatch',
            'highlight' => 'ch',
            'audio' => 'ch_rev/snatch.m4a',
            'tip' => null,
        ],
        [
            'image' => 'phonics_l2/chrev/beach.png',
            'imageSize' => 'w-[20vw]',
            'options' => ['teach', 'beach'],
            'answer' => 'beach',
            'highlight' => 'ch',
            'audio' => 'ch_rev/beach.m4a',
            'tip' => null,
        ],
        [
            'image' => 'phonics_l2/chrev/match.png',
            'imageSize' => 'w-[20vw]',
            'options' => ['hatch', 'match'],
            'answer' => 'match',
            'highlight' => 'ch',
            'audio' => 'ch_rev/match.m4a',
            'tip' => null,
        ],
        [
            'image' => 'phonics_l2/chrev/couch.png',
            'imageSize' => 'w-[20vw]',
            'options' => ['slouch', 'couch'],
            'answer' => 'couch',
            'highlight' => 'ch',
            'audio' => 'ch_rev/couch.m4a',
            'tip' => null,
        ],
    ];
@endphp
...
@section('content')
    ...
    @foreach ($panels as $panel)
        ...
        <div class="phonics-panel answer flex flex-col justify-center space-y-[2vw] items-center w-full"
            ...
            {{-- Answer word with highlighted digraph --}}
            {{-- Answer word with highlighted digraph --}}
            @php
                $highlight = $panel['highlight'];
                $answer = $panel['answer'];
                $highlightPos = strpos($answer, $highlight);

                // Split the word into parts
                if ($highlightPos === 0) {
                    // Highlight is at the beginning
                    $before = '';
                    $after = substr($answer, strlen($highlight));
                } elseif ($highlightPos === strlen($answer) - strlen($highlight)) {
                    // Highlight is at the end
                    $before = substr($answer, 0, $highlightPos);
                    $after = '';
                } else {
                    // Highlight is in the middle
                    $before = substr($answer, 0, $highlightPos);
                    $after = substr($answer, $highlightPos + strlen($highlight));
                }
            @endphp
            <h2 class="text-[4vw]">
                <span class="text-white">{{ $before }}</span><span
                    class="text-[#f7b94a]">{{ $highlight }}</span><span class="text-white">{{ $after }}</span>
            </h2>
        </div>
    @endforeach
