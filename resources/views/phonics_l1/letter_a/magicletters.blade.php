...
@section('content')
    ...
    <div class="phonics-panel">
        <div class="flex">
            ...
            <a href="" class="w-[27vw] h-fit info-btn2">
...
@push('script')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const closeBtn = wellDonePanel.querySelector('#retry'); // Close button in Well Done panel

            // Get all letter buttons
            const falseLetters = document.querySelectorAll('.phonics-panel .grid a:not(.col-span-2 a)');
            const trueLetters = document.querySelectorAll('.phonics-panel .col-span-2 a');

            // Create audio elements
            const tryAgainSound = new Audio('{{ asset('assets/audio/phonic_audio/tryagain.mp3') }}');
            const wellDoneSound = new Audio('{{ asset('assets/audio/phonic_audio/welldone.mp3') }}');
            ...
            // Handle FALSE letter clicks (first two letters)
            falseLetters.forEach(letter => {
                letter.addEventListener('click', function(e) {
                    e.preventDefault();
                    tryAgainPanel.style.display = 'flex';
                    playSound(tryAgainSound);
                });
            });

            // Handle TRUE letter click (last letter - 'a')
            trueLetters.forEach(letter => {
                letter.addEventListener('click', function(e) {
                    e.preventDefault();
                    wellDonePanel.style.display = 'flex';
                    playSound(wellDoneSound);
                });
            });
            ...
            closeBtn.addEventListener('click', function(e) {
                e.preventDefault();
                window.location.href = '{{ url('/phonics/letter_a') }}'; // Replace with your route
            });
            ...
            if (soundButton) {
                soundButton.addEventListener('click', function() {
                    ...
                    const letterSound = new Audio('{{ asset('sounds/letters/') }}' + letter + '.mp3');
